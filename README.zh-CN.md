...
**全屏时自动隐藏侧边栏。** Chrome 自己不会在视频全屏时收起侧边栏，只会把全屏内容塞进侧边栏旁边剩下的地方。所以进全屏时由扩展主动关闭，退出时再打开。退出后的恢复是尽力而为：Chrome 要求打开侧边栏必须有用户手势，而用 Esc 退出全屏并不带手势。如果被拒绝，侧边栏就保持关闭，点工具栏图标即可打开。
...
| 动作 | 花什么 |
| --- | --- |
| 点侧边栏里的「获取字幕」 | Supadata 额度，每个视频 1 个 |
| 按播放器上的字幕按钮 | Supadata（如果还没取过字幕）加 DeepSeek 翻译费 |
| 打开侧边栏、切换视频、刷新、滚动、做笔记之外的浏览 | 什么都不花 |

字幕原文和翻译是两笔独立的开销，所以两边都要你先开口。侧边栏打开一个没取过字幕的视频时，只显示标题和一个「获取字幕」按钮，**不会自动去取**。中文视频，或者你只是想看看笔记，就一分钱都不花。已经取过的视频（最近 20 个、30 天内）直接从缓存打开，因为那是免费的。
